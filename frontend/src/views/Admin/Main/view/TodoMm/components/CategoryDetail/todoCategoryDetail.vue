...
watch(visible, async (v) => {
  if (v && categoryId.value) {
    const r = await axios.get('/admin/todoMm/category/detail', { params: { id: categoryId.value } });
...
const onSave = async () => {
  ...
  const r = await axios.put('/admin/todoMm/category/update', {
    id: detail.value.id,
...
<template>
  <Dialog v-model:visible="visible" :draggable="false" modal header="待做分类详情" :style="{ width: '650px'}">
    <div v-if="detail">
      <el-form label-width="100px" class="form-container" label-position="left">
        ...
        <el-form-item label="类型"><el-input-number v-model="detail.type" :min="0" :step="1" /></el-form-item>
