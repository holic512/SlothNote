...
public class AccountSettingsController {
    ...
    public ApiResponse<UserAllProfileDTO> getAll(@RequestHeader(value = "satoken", required = false) String token) {
    ...
    @PutMapping("/nickname")
    public ApiResponse<Void> nickname(@RequestHeader(value = "satoken", required = false) String token, @RequestBody NicknameDTO body) {
        service.updateNickname(token, body.getNickname());
        return new ApiResponse.Builder<Void>().status(200).message("ok").build();
    }

    @PutMapping("/gender")
    public ApiResponse<Void> gender(@RequestHeader(value = "satoken", required = false) String token, @RequestBody GenderDTO body) {
        service.updateGender(token, body.getGender());
        return new ApiResponse.Builder<Void>().status(200).message("ok").build();
    }

    @PutMapping("/age")
    public ApiResponse<Void> age(@RequestHeader(value = "satoken", required = false) String token, @RequestBody AgeDTO body) {
        service.updateAge(token, body.getAge());
        return new ApiResponse.Builder<Void>().status(200).message("ok").build();
    }

    @PutMapping("/contactInfo")
    public ApiResponse<Void> contactInfo(@RequestHeader(value = "satoken", required = false) String token, @RequestBody ContactInfoDTO body) {
        service.updateContactInfo(token, body.getContactInfo());
        return new ApiResponse.Builder<Void>().status(200).message("ok").build();
    }

    @PutMapping("/bio")
    public ApiResponse<Void> bio(@RequestHeader(value = "satoken", required = false) String token, @RequestBody BioDTO body) {
        service.updateBio(token, body.getBio());
        return new ApiResponse.Builder<Void>().status(200).message("ok").build();
    }

    @PostMapping(path = "/avatar", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ApiResponse<String> avatar(@RequestHeader(value = "satoken", required = false) String token, @RequestPart("file") MultipartFile file) throws IOException {
        String url = service.uploadAvatar(token, file);
        return new ApiResponse.Builder<String>().status(200).message("ok").data(url).build();
    }

    @PutMapping("/password")
    public ApiResponse<Void> changePassword(@RequestHeader(value = "satoken", required = false) String token, @RequestBody Map<String, String> body) {
